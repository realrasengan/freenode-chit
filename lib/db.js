...
class Database {
  ...
  async getReplies(parent) {
    let results = await this.db.all("SELECT * FROM CHITS WHERE PARENT = ? AND ISDELETED=0 ORDER BY TIMESTAMP DESC LIMIT "+constants.PAGE_LIMIT,parent);
    return results;
  }
  ...
  // Chit
  // Returns last id or 0
  async reply(nick,msg,parent) {
    if(this.findChit(parent)) {
      let results = await this.db.run("INSERT INTO CHITS (NICK,CHIT,TIMESTAMP,ISDELETED,PARENT) VALUES (?,?,?,?,?)",
                        nick.toLowerCase(),msg,Math.floor(Date.now()/1000),0,parent);
      if(results.lastID)
        return results.lastID;
      return 0;
    }
